...
class UserController
{
    ...
    public function invite()
    {
        ...
        try {
            ...
            // Also keep a record in the invitations table for auditing/tracking
            $stmtInvite = $this->db->prepare("
                INSERT INTO invitations (email, token, role, branch_id, expires_at)
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmtInvite->execute([$email, $token, $role, $branchId, $expires]);
        } catch (PDOException $e) {
            sendError('Database error: ' . $e->getMessage(), 500);
            return;
        }

        // Build URL dynamically so it works on any server (localhost or production)
        ...
        $message = "
            <html><body style='font-family:Arial,sans-serif;color:#111827;line-height:1.5;'>
                ...
                <p>This invitation link expires in <strong>24 hours</strong> &mdash; by <strong>" . date('D, d M Y \a\t H:i', strtotime($expires)) . "</strong>.</p>
        ...
        if (!$emailSent) {
            ...
            sendSuccess(
                ['link' => $setupLink], 
                'Invitation created! Email delivery failed, but the setup link has been logged in backend/logs/invite.log'
            );
        } else {
